...
import { Package, User as UserIcon, MapPin, Edit2, Save, X, Gift, Award, CheckCircle } from 'lucide-react';
          ...
          <div className="col-span-1">
            <div className="bg-white shadow-md rounded-lg p-6 mb-6 border border-neutral-100 sticky top-24">
              ...
              <div className="border-t border-neutral-200 pt-4">
                 <button className="w-full text-left px-4 py-3 bg-amber-50 text-amber-800 rounded-md font-bold flex items-center border border-amber-100"><Package className="mr-3" size={18}/> Orders History</button>
              </div>
            </div>
          </div>
            ...
            <div className="bg-white shadow-md rounded-lg p-6 border border-neutral-100">
              ...
              {orders.length === 0 ? (
                ...
                <div className="space-y-6">
                  {orders.map(order => {
                    const currentStatus = getDerivedStatus(order);
                    return (
                      <div key={order.id} className="border border-neutral-200 rounded-lg overflow-hidden">
                        <div className="bg-neutral-50 px-6 py-4 border-b border-neutral-200 flex justify-between items-center flex-wrap gap-4">
                          ...
                          <div>
                             <p className="text-xs text-neutral-500 uppercase font-bold">Delivery Date</p>
                             <p className={`font-medium ${currentStatus === 'Delivered' ? 'text-green-600' : 'text-amber-700'}`}>
                                 {order.deliveryDate}
                             </p>
                          </div>
                          <div className="text-right">
                             <p className="text-xs text-neutral-500 uppercase font-bold">Total Paid</p>
                        ...
                        <div className="px-6 py-4">
                          <ul className="divide-y divide-neutral-100">
                           ...
                           <div className="mt-4 flex justify-between items-center pt-4 border-t border-neutral-100">
                               <div className="flex items-center gap-2">
                                   <span className={`inline-flex items-center px-3 py-1 rounded-full text-xs font-bold ${currentStatus === 'Delivered' ? 'bg-green-100 text-green-800' : 'bg-blue-100 text-blue-800'}`}>
                                      {currentStatus === 'Delivered' && <CheckCircle size={12} className="mr-1"/>}
